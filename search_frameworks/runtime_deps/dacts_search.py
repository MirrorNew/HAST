...
All artifacts produced by this local copy are written under:
HAST2026/main/runs/DACTS-style/

The runner intentionally keeps the old PUCT experiment spirit:
...
EXPERIMENT_ROOT = Path(__file__).resolve().parents[2] / "runs" / "DACTS-style"
OUTPUT_DIR = EXPERIMENT_ROOT / "outputs"
CANDIDATE_DIR = EXPERIMENT_ROOT / "candidates"
...
def generate_powerlaw_network(
    ...
    """Generate a power-law graph using the same configuration-model idea as old PUCT."""
...
def llm_mutate_config(
    ...
    return asyncio.run(llm_mutate_config_async(parent, diagnostics, records, rng, model))
...
def run_search(args: argparse.Namespace) -> None:
    ...
    if args.reuse_graphs and graphs_path.exists():
        ...
        graphs = [json_graph.node_link_graph(item) for item in graphs_payload]
    else:
    ...
    while len(records) < args.nodes_to_search:
        ...
        if args.use_llm and (len(records) % max(1, args.llm_every) == 0):
            child = llm_mutate_config(parent_cfg, diags.get(clade, {}), records, rng, args.model)
        if child is None:
...
def write_algorithm_flow() -> None:
    lines = [
        ...
        "",
        "## Changes From Old PUCT",
        "",
        "- Root: old PUCT could start from broad seeds or prior candidates; this version starts only from original HDA.",
        "- Search unit: old PUCT mainly searched candidate implementations/variants; this version searches typed dismantling-program configurations that compile into executable algorithms.",
        "- Selection: old PUCT uses node-level UCB/PUCT pressure; this version uses clade-level budget allocation so structurally different ideas receive exploration budget.",
        "- Feedback: old PUCT mostly uses scalar reward; this version exposes diagnostic feedback to mutation: scale pressure, fragmentation pressure, and speed pressure.",

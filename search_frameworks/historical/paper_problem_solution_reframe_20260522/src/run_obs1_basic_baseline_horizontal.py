...
BASIC_METHODS = ["HDA", "CI", "DC", "KCore", "CLUC"]
DATASETS = ["Grid", "Collaboration", "HepPh", "email", "crime", "CEnew", "Yeast"]
EXTENDED_RECORD_DIR = ROOT / "paper_tables" / "extended_12graph_records"
SEQUENCE_DIR = EXTENDED_RECORD_DIR / "python_baseline_sequences"
ORDER_METHOD = {"KCore": "KCORE"}
SEQUENCE_METHOD = {"HDA": "hda", "CI": "ci", "DC": "dc", "KCore": "kcore", "CLUC": "cluc"}
FIXED_CASES = [
    {"dataset": "Collaboration", "method_a": "HDA", "method_b": "CLUC", "step_a": 931, "step_b": 1243},
    {"dataset": "Collaboration", "method_a": "HDA", "method_b": "CI", "step_a": 781, "step_b": 601},
...
def load_order(dataset: str, method: str) -> list[int]:
    sequence_name = SEQUENCE_METHOD.get(method)
    if sequence_name is not None:
        sequence_path = SEQUENCE_DIR / f"{dataset}__{sequence_name}.csv"
        if sequence_path.exists():
            df = pd.read_csv(sequence_path, encoding="utf-8-sig")
            df = df[df["step"].astype(int) > 0].sort_values("step")
            return [int(x) for x in df["node"].tolist()]
    path = RECORD_DIR / f"{dataset}_{ORDER_METHOD.get(method, method)}_steps.csv"
...
def find_cases() -> list[dict[str, object]]:
    ...
    for dataset in DATASETS:
        ...
        for method in BASIC_METHODS:
            path = EXTENDED_RECORD_DIR / f"{dataset}__{method}.csv"
            if path.exists():
